...
def aggregatepropensities(data, idx, k, features, pi, eta, clip = True):
    propensities = [np.clip(pi[i].propensity(data, features), eta, 1 - eta) if clip else pi[i].propensity(data, features) for i in range(k) if i != idx] # Skip own index for cross-validation
...
def aggregateregression(data, idx, k, features, mu, b, clip = True):
    mu0 = [np.clip(mu[i].control(data, features), -b, b) if clip else mu[i].control(data, features) for i in range(k) if i != idx] # Skip own index for cross-validation
    mu1 = [np.clip(mu[i].treated(data, features), -b, b) if clip else mu[i].treated(data, features) for i in range(k) if i != idx] # Skip own index for cross-validation
...
class EnsembleEstimator:
    ...
    def __init__(self, n, k, features, eta = 0.1, b = 1, sigma = 0, estimator = 'AIPW', binary = False):
        ...
        self.eta = eta
        self.b = b
        self.model_size = n // k # Size of the small groups. Some have 1 more element
        self.sigma = sigma
    ...
    def fit_estimators(self, data, propensity_trainer, regression_trainer):
        ...
        self.groups = [data.loc[i] for i in np.array_split(data.index, self.k)]
